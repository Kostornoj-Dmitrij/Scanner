...
def create_res_file(message, form):
    ...
    if form == 'pdf':
        ...
        with tempfile.NamedTemporaryFile(suffix=true_suffix, delete=False, mode='w', encoding='utf-8') as temp_file:
            temp_file.write(message.document)
...
def save_file_with_unique_name(downloaded_file, original_filename):
...
def handle_content(message):
    ...
    if message.content_type == 'document':
        if message.document.file_name.lower().endswith(('.jpg', '.jpeg', '.img')):
            file_id = message.document.file_id
            file_info = bot.get_file(file_id)
            file_path = file_info.file_path
            downloaded_file = bot.download_file(file_path)
            with open(r'D:\3.jpg', 'wb') as new_file:
                new_file.write(downloaded_file)
...
@bot.message_handler(func=lambda message: True)
def ask_for_result_format(message):
    user_id = message.chat.id
    keyboard = types.InlineKeyboardMarkup()
    keyboard.row(
